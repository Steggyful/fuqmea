...
// Meme list: gallery.json is the static catalog of repo-hosted memes; we then
// merge the public meme_entries table from Supabase to apply admin renames,
// hide flagged static memes, and append admin-uploaded memes (hosted on R2).

const GALLERY_JSON_URL = 'assets/images/gallery.json';
const SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_s70S_HrNIZHzbjaiyXChsw_aE8CMQbE';
...
/** src → { displayName, source } for memes that have admin overrides or are R2 uploads. */
const _memeOverrides = new Map();
...
// Accepts either a repo-relative static path or an R2-hosted upload URL.
function validateImagePath(path) {
  if (typeof path !== 'string') return false;
  if (path.includes('..')) return false;
  if (path.startsWith(R2_PUBLIC_BASE + '/uploads/')) {
    try {
      const u = new URL(path);
      return u.hostname === 'memes.fuqmea.com' && u.pathname.startsWith('/uploads/');
    } catch (_) {
      return false;
    }
  }
  if (path.startsWith('assets/images/') && !path.includes('//')) return true;
  return false;
}
...
async function fetchPublicMemeEntries() {
  try {
    const url =
      `${SUPABASE_URL}/rest/v1/meme_entries` +
      '?select=source,storage_key,display_name,extension,hidden' +
...
async function loadGalleryList() {
  ...
  galleryListLoadPromise = (async () => {
    let staticList = FALLBACK_GALLERY_PATHS.filter(validateImagePath);
    ...
    try {
      ...
      if (res.ok) {
        ...
        const validated = list.filter(validateImagePath);
        if (validated.length > 0) staticList = validated;
      }
    } catch (_) {
      console.warn('gallery.json load failed, using fallback');
    }

    // Merge admin-managed overrides (renames, hides, uploads). If the fetch
    // fails or returns nothing, the static list still works on its own.
    const entries = await fetchPublicMemeEntries();
    const hiddenStatic = new Set();
    const uploads = [];
    let featuredUpload = null;

    entries.forEach((entry) => {
      if (!entry || typeof entry.storage_key !== 'string') return;
      if (entry.source === 'static') {
        if (entry.hidden) {
          hiddenStatic.add(entry.storage_key);
          return;
        }
        if (validateImagePath(entry.storage_key)) {
          _memeOverrides.set(entry.storage_key, {
            displayName: entry.display_name,
            source: 'static',
          });
        }
      } else if (entry.source === 'upload' && !entry.hidden) {
        const src = `${R2_PUBLIC_BASE}/${entry.storage_key}`;
        if (!validateImagePath(src)) return;
        uploads.push(src);
        _memeOverrides.set(src, {
          displayName: entry.display_name,
          source: 'upload',
        });
      }
    ...
    let merged = staticList.filter((p) => !hiddenStatic.has(p));
    merged = [...merged, ...uploads];

    return merged.length > 0 ? merged : FALLBACK_GALLERY_PATHS.filter(validateImagePath);
  })();
...
function filenameToDisplay(src) {
  const override = _memeOverrides.get(src);
  if (override) return override.displayName.replace(/[-_]/g, ' ').trim();
...
function extractCategories(src) {
  // If admin renamed the meme, derive categories from the override name instead
  // of the original filename (filename-based extraction would still see the old prefix).
  const override = _memeOverrides.get(src);
  const name = override
  ...
  if (typeof FuqmeaCategories !== 'undefined' && FuqmeaCategories.extractCategories) {
    // FuqmeaCategories expects a path-like input; synthesize one when we have an override.
    return FuqmeaCategories.extractCategories(override ? `x/${name}.x` : src);
  }
...
const SUPABASE_URL = 'https://mxwrpjyiwcmdzlymmnmr.supabase.co';
...
async function checkTikTokLive(username) {
  try {
    const res = await fetchWithTimeout(
      `${SUPABASE_URL}/functions/v1/check-tiktok-live`,
      {
        method: 'POST',
    ...
    if (!res.ok) return { live: false };
    const data = await res.json();
    if (!data.live) return { live: false };
    return { live: true, viewers: data.viewers ?? null, platform: 'tiktok' };
  } catch (e) {
