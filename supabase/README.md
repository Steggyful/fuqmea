...
Canonical definitions live in **[`schema.sql`](schema.sql)** end-to-end. **[`supabase/migrations/`](migrations/)** holds **incremental** SQL meant to match that file when applied **in timestamp order** (e.g. `supabase db push`).

The folder is the source of truth for what exists beyond the base schema: leaderboard aggregates and RPCs, wagered columns, rakeback, display-name moderation, arcade streaks, quest state, avatar URLs, device-wallet import shape fixes, `apply_settlement` evolution (streak/daily/aura/crash), and occasional operational resets. **Open [`migrations/`](migrations/)** for the full list—do not rely on an outdated enumerated table in docs.

**SQL Editor workflow:** one full paste of **`schema.sql`** is sufficient for a fresh project. **CLI migrations** do not recreate everything from zero — run **`schema.sql`** once on a new project, then apply migrations for incremental changes, or re-paste the full **`schema.sql`** when you intentionally merge upstream changes.
...
Weekly scope uses **`date_trunc('week', now())`** — week starts Monday, in the database session timezone (**usually UTC** on Supabase). **Weekly quests** (and `wallets.quest_state.weekly.week`) use **`weekKey()`** in **`games.js`** (Mountain Time calendar → `YYYY-Www`), so LB weekly vs quest weekly are intentionally different scopes until aligned in SQL.
