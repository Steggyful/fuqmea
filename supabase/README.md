...
| [`20260505130000_quest_state_cross_device.sql`](migrations/20260505130000_quest_state_cross_device.sql) | `wallets.quest_state jsonb`, `merge_quest_state(jsonb)`, `apply_settlement_quest_claim(...)` (idempotent quest payouts), extended `import_initial_device_wallet` return with `quest_state`. Quest week/day keys follow **Mountain Time** in `games.js`; leaderboard weekly aggregates may still use **UTC week** in Postgres — different scopes. Deploy migration + **`settle-game`** Edge + **`cloud-sync.js@1.24.0`**. |
