-- Lifetime rakeback: a non-decreasing per-wallet counter that tracks total
-- rakeback ever accrued, in parallel to the existing claimable `rakeback_pool`.
-- Pool resets on claim; lifetime never decreases. Idempotent: safe to re-run.

alter table public.wallets
  add column if not exists rakeback_lifetime numeric(12,2) not null default 0
    check (rakeback_lifetime >= 0);

-- Backfill: existing accounts have at least their current pool's worth of
-- accrual on record. We can't reconstruct pre-migration claim history, so
-- start lifetime at the current pool value for any row where it's still 0.
update public.wallets
set rakeback_lifetime = rakeback_pool
where rakeback_lifetime = 0 and rakeback_pool > 0;

-- Replace apply_settlement so the return shape includes lifetime and the
-- accrual writes to both columns. Signature and parameter list are unchanged.
...
-- claim_rakeback: surface lifetime in the return shape so the client can paint
-- the persistent stat in the same response. Lifetime is intentionally untouched
-- on claim.
